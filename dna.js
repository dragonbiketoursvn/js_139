/*
  Write a program that can calculate the Hamming distance between two DNA strands.

  A mutation is simply a mistake that occurs during the creation or copying of a nucleic acid, in particular DNA. 
  Because nucleic acids are vital to cellular functions, mutations tend to cause a ripple effect throughout the cell. 
  Although mutations are technically mistakes, a very rare mutation may equip the cell with a beneficial attribute. 
  In fact, the macro effects of evolution are attributable by the accumulated result 
  of beneficial microscopic mutations over many generations.

  -The simplest and most common type of nucleic acid mutation is a point mutation, which replaces one base with another at a single nucleotide.

  By counting the number of differences between two homologous DNA strands taken from different genomes 
  with a common ancestor, we get a measure of the minimum number of point mutations that could have occurred on the evolutionary path between the two strands.

  This is called the Hamming distance.

  The Hamming distance between these two DNA strands is 7.

  The Hamming distance is only defined for sequences of equal length. 
  If you have two sequences of unequal length, you should compute the Hamming distance over the shorter length.
*/

/*
  class DNA methods:
  -constructor (takes string of nucleotides as arg and assigns to strand)
  -hammingDistance (takes string of nucleotides and compares against strand property)
*/


class DNA {
  constructor(string) {
    this.strand = string;
  }
  hammingDistance(comparisonStrand) {
    let lengthToCompare = Math.min(this.strand.length, comparisonStrand.length);
    let hammingDistance = 0;

    for (let idx = 0; idx < lengthToCompare; idx += 1) {
      if (this.strand[idx] !== comparisonStrand[idx]) {
        hammingDistance += 1;
      }
    }
    return hammingDistance;
  }
}

module.exports = DNA;